(function(root){
 ...
 const partyAmount=(s,n)=>n===350?(current(s)?.partyIncome??350):(current(s)?.refundDue??150);
 ...
 function event(s){
  ...
  switch(s.event){
   case 'fundraising':return {speaker:'後援会長のオオクボ',title:'政治資金パーティを開きませんか？',body:'開催した場合だけ資金と人脈が増え、体力と今回の主行動を使います。ここで見送っても資金・体力は減りません。大規模開催では、後日、支援した業界団体から公約を求められます。'+(s.mandate?'今回は党務のため開催できません。':s.party===1&&s.nomination!=='無所属'?'都市改革党の規則では個人参加型のみ開催できます。':'人脈は「地域交流」や「後援会づくり」で育ちます。'),choices:[
    c('small','小規模パーティを開催する','開催後：資金＋'+G.funds(s,150)+'万円・人脈＋3／体力−15・今回の主行動を消費（必要：人脈20・体力15）',s.network<20||s.energy<15||s.mandate),
    c('large','大規模パーティを開催する',(s.memory[11]?'業界との関係が破綻／':'')+'開催後：資金＋'+G.funds(s,350)+'万円・人脈＋5／体力−25・今回の主行動を消費／後日、業界団体から要求あり（必要：人脈35・体力25）',s.network<35||s.energy<25||s.mandate||s.memory[11]||(s.party===1&&s.nomination!=='無所属')),
   ...
   case 'sponsor_request':return {speaker:'パーティを支えた業界団体',title:'大規模パーティの支援者から公約の要求',body:'大規模パーティ開催時に、資金'+partyAmount(s,350)+'万円はすでに受け取っています。今回はその支援への対応です。公約を受け入れるか、受け取った資金の一部を返すか、返金せず関係を断つか選びます。',choices:[
    c('pledge','公約を受け入れる（返金なし）','資金は変化なし／業界好感度＋4／都市無党派−10／約束を履行済みにする'),
    c('refund','受け取った支援から'+partyAmount(s,150)+'万円を返す','大規模開催で得た'+partyAmount(s,350)+'万円のうち'+partyAmount(s,150)+'万円を返金／資金−'+partyAmount(s,150)+'万円・業界好感度−2／約束を解消',s.money<partyAmount(s,150)),
   ...
   case 'influencer':return {speaker:'動画クリエイターのアオイ',title:'あなたの政策、動画で届けませんか？',body:'若い世代へ届くコラボの提案です。継続契約では発信方針も選びます。休養を含む主行動1回ごとに配信し、3回で契約終了。',choices:[
    c('single','単発コラボを依頼する',''+G.funds(s,150)+'万円／知名度＋5／若者好感度＋3',s.money<G.funds(s,150)),
    c('ongoing','3回の継続契約を結ぶ',''+G.funds(s,300)+'万円／毎回知名度＋3／次に発信方針を選択',s.money<G.funds(s,300)),
   ...
   case 'consultant':return {speaker:'選挙コンサルのミズノ',title:'最後の選挙戦、陣営を強化しましょう。',body:''+G.funds(s,150)+'万円で主行動3回を強化。街頭演説・SNS・広告の知名度上昇量が50%増え、主行動の消費体力は25%減少します。休養も1回として数えます。今回の選挙限りの契約です。',choices:[
    c('hire','選挙対策パックを契約する',''+G.funds(s,150)+'万円／主行動3回／知名度効果＋50%・消費体力−25%',s.money<G.funds(s,150)),
    c('skip','自分たちで戦う','資金を温存する')
   ]};
  }
  return base.event(s);
 }
 const costs={staff:4,outreach:10,donors:12,practice:8,speech:15,visit:12,org:10,network:10,study:8,media:15,sns:8,hq:8,rest:0,ad:5,question:12,committee:8,local:10,faction:8,abroad:8,study_group:8,stump:5};
 ...
 function consumeParty(s,large){
  ...
  p.partyIncome=G.funds(s,large?350:150);p.refundDue=G.funds(s,150);s.money+=p.partyIncome;s.network=clamp(s.network+(large?5:3));s.energy-=cost;
  if(large){debt(s,11,3);p.sponsorDebt=s.debts.at(-1).id;s.debts.at(-1).name='パーティ支援業界団体';}
  log(s,(large?'大規模':'小規模')+'パーティを開催。主行動を消費、差引資金＋'+G.funds(s,large?350:150)+'万円。');
  afterAction(s,'fundraising');
  const exhausted=s.energy===0;if(exhausted){s.energy=0;s.fame=clamp(s.fame-3);log(s,'体力が尽き、休養した。');}
 ...
 function choose(s,id){
  ...
  if(type==='sponsor_request'){
   ...
   if(id==='refund'){s.money-=partyAmount(s,150);s.likes[11]=clamp(s.likes[11]-2,-50,100);d.status='履行済み';}
  ...
  if(type==='influencer'){
   mark(p,type);
   if(id==='single'){s.money-=G.funds(s,150);s.fame=clamp(s.fame+5);s.likes[2]=clamp(s.likes[2]+3,-50,100);}
   if(id==='ongoing'){s.money-=G.funds(s,300);p.influencer=3;s.stage='event';s.event='influencer_direction';}
  }
  ...
  if(type==='consultant'){mark(p,type);if(id==='hire'){s.money-=G.funds(s,150);p.consultant=3;}}
 ...
 function validate(s){
  ...
  for(const key of ['partyIncome','refundDue'])if(p[key]!==undefined&&(!Number.isFinite(p[key])||p[key]<0||p[key]>1000000))return false;
