...
function settleBudget(s){const b=budget(s);if(!b.income)return;s.money+=b.net;log(s,'定期収支：個人支援＋'+b.income+'万円／事務所・スタッフ費−'+b.expense+'万円（差引＋'+b.net+'万円）');}
function actionFunds(s,id){return funds(s,{network:50,ad:100,study_group:100,staff:80,publicity:100}[id]||0);}
function random(s){if(previewRandom.has(s))return previewRandom.get(s)();s.seed=(Math.imul(s.seed,1664525)+1013904223)>>>0;return s.seed/4294967296}
...
function allowed(s,id){return actions.some(a=>a[0]===id)&&s.stage==='main'&&s.money>=actionFunds(s,id)&&!(id!=='rest'&&s.energy===0)&&!(s.mandate&&id!=='hq'&&id!=='rest')&&!(id==='media'&&s.fame<30)&&!(id==='network'&&s.money<funds(s,50))&&!(id==='ad'&&(!campaign(s)||s.money<funds(s,100)))&&!(id==='hq'&&s.nomination==='無所属');}
function action(s,id,target=0,business=false){if(!allowed(s,id))return false;if(!Number.isInteger(target)||target<0||target>11)return false;if(id==='org'&&(groups[target][5]<3||s.memory[target]))return false;let cost={staff:4,outreach:10,donors:12,practice:8,speech:15,visit:12,org:10,network:10,study:8,media:15,sns:8,hq:8,rest:0,ad:5}[id];if(cost===undefined)return false;const forced=s.mandate;s.mandate=false;const multi=campaign(s)?2:1;if(id==='rest'){s.energy=clamp(s.energy+35)}else{s.energy=clamp(s.energy-(root.Game.Campaign?.energyCost(s,id)??cost*multi))}
if(id==='staff'){s.money-=funds(s,80);change(s,groups.map((_,i)=>[i,1.25]));s.network+=2;}
if(id==='outreach'){s.network+=5;s.speech+=1;}
if(id==='donors'){s.money+=funds(s,60+Math.floor((s.network+s.fame)/5));s.network+=1;}
if(id==='practice')s.speech+=5;
...
case 'collect':{const d=s.debts[s.collection[0]];return {speaker:d.name+'からの連絡',title:d.group<0?'「次の採決は、党に従ってください」':'「当選したのだから、約束をお願いします」',body:d.group<0?'比例で助けた党から、方針への協力を求められました。地元の支持者とは意見が違います。':d.name+'が、自分たちを優先する政策への協力を求めています。応じれば、別の支持者に負担が回ります。',choices:[c('fulfill','約束を果たす',d.name+'が喜ぶ／対立する支持層が怒る'),c('partial','一部の協力にとどめる','資金'+funds(s,200)+'万円で調整／約束が1件残る',s.money<funds(s,200)),c('break','要求を断る','相手が離反／組織票を失う／信頼が戻りにくくなる')]};}
