...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (header) {
    const onScroll = () => {
      if (window.scrollY > 12) {
        header.classList.add('bg-washi-50/95', 'shadow-md', 'backdrop-blur');
        header.classList.remove('bg-transparent');
      } else if (header.dataset.transparentHero === 'true') {
        header.classList.remove('bg-washi-50/95', 'shadow-md', 'backdrop-blur');
  ...
  document.querySelectorAll('[data-nav-link]').forEach((link) => {
    ...
    if (target === currentPage) {
      link.classList.add('text-shu-600', 'font-bold');
      link.classList.remove('text-sumi-700');
    }
  });
