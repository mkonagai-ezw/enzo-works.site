// うちぬき酒造株式会社 site — shared behaviour
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (header) {
    const onScroll = () => {
      if (window.scrollY > 12) {
        header.classList.add('bg-cream-50/95', 'shadow-md', 'backdrop-blur');
        header.classList.remove('bg-transparent');
      } else if (header.dataset.transparentHero === 'true') {
        header.classList.remove('bg-cream-50/95', 'shadow-md', 'backdrop-blur');
  ...
  document.querySelectorAll('[data-nav-link]').forEach((link) => {
    ...
    if (target === currentPage) {
      link.classList.add('text-gold-600', 'font-bold');
