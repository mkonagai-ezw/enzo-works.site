// トレカ基地 西条 site — shared behaviour
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (header) {
    const onScroll = () => {
      if (window.scrollY > 12) {
        header.classList.add('bg-night-900/95', 'shadow-md', 'backdrop-blur');
        header.classList.remove('bg-transparent');
      } else if (header.dataset.transparentHero === 'true') {
        header.classList.remove('bg-night-900/95', 'shadow-md', 'backdrop-blur');
  ...
  document.querySelectorAll('[data-nav-link]').forEach((link) => {
    ...
    if (target === currentPage) {
      link.classList.add('text-lime-400', 'font-bold');
      link.classList.remove('text-night-100');
    }
  });

  /* ---------- Buyback price tab switcher ---------- */
